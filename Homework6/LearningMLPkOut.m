function [netw, iteration, RMSE] = LearningMLPkOut(Input, nhiddenneurons, noutputs, errorlimit, iterationslimit, OutputFlag)
    ...
    A = Input;
    % function call for reassignment of desired ouputs
  %  A = reassignment(Input,noutputs);
    %disp(A);
    % N is now the number of learning samples
    % ninputs is now the number of inputs
    [N,ninputs]=size(A);
    ninputs=ninputs-noutputs;

    % An array to store actual outputs after learning
    ActualOutputs = zeros(noutputs, N);

    % extraction of input samples (only inputs)
    inputs=A(:,1:ninputs);
    % extraction of the desired outputs
    targets=A(:,ninputs+1:end);
    ...
    % learning continues as long as error>errorlimit and iteration<=iterationslimit
        ...
        error = sum((ActualOutputs - targets').^2)/noutputs;
        error1 = sum(error)/N;
        % RMSE
        RMSE = sqrt(error1);
